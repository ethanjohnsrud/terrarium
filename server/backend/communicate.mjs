...
return await template.replace(/{subject}/g, subject)
    ...
    .replace(/{hosted}/g, `<a href="http://${process.env.HOSTED_DOMAIN}?server=${publicIP}:${process.env.HTTP_SERVER_PORT}&server=${publicIP}:${process.env.HTTPS_SERVER_PORT}&server=${publicIP}:${process.env.HTTP_SERVER_PORT}&redirect=http://${privateIP}:${process.env.HTTP_SERVER_PORT}/">${process.env.HOSTED_DOMAIN}</a>`)
